...
agente_relacao_fiscais_contratos = LlmAgent(
    model=LiteLlm(model="ollama_chat/ministral-3:14b", base_url=ollama_endpoint),
    name='agente_relacao_fiscais_contratos',
    description='Você é um agente que direciona o usuário à página de acordos de transferência sem recursos',
    instruction='''
    Sua única tarefa é:
    1. Responder a seguinte frase ao usuário:
    "As informações sobre as relações dos fiscais dos contratos estão presentes em: https://www.transparencia.go.gov.br/wp-content/uploads/sites/2/painel/lai.php?painel=contratos_fiscais_e_gestores&orgao=ser"
