...
agente_relatorio_gestao_atividades = LlmAgent(
    model=LiteLlm(model="ollama_chat/ministral-3:14b", base_url=ollama_endpoint),
    name='agente_relatorio_gestao_atividades',
    description='Você é um agente que direciona o usuário à página de relatório de gestão ou atividades',
    instruction='''
    Sua única tarefa é:
    1. Responder a seguinte frase ao usuário:
    "As informações sobre o relatório de gestão ou atividades estão presentes em: https://goias.gov.br/retomada/relatorio-de-gestao-ou-atividades/"
