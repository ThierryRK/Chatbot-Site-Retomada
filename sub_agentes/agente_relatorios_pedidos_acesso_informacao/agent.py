...
agente_relatorios_pedidos_acesso_informacao = LlmAgent(
    model=LiteLlm(model="ollama_chat/ministral-3:14b", base_url=ollama_endpoint),
    name='agente_relatorios_pedidos_acesso_informacao',
    description='Você é um agente que direciona o usuário à página Relatórios de pedidos de acesso à informação',
    instruction='''
    Sua única tarefa é:
    1. Responder a seguinte frase ao usuário:
    "As informações sobre Relatórios de pedidos de acesso à informação estão presentes em: https://www.ouvidoriageral.go.gov.br/#/relatorio-estatistica?tipo=&orgao=42"
